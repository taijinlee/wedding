...
lint: build-common
	find . -type f -a ! -path './mytests/*' -a ! -path './.git/*' -a ! -path './web/*' -a ! -path './web-build/*' -a ! -path './node_modules/*' -a ! -path './vendor/*' -exec ./node_modules/jshint/bin/hint {} --config config/build/jshint.config.json \;
